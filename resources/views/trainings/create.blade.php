                ...
                @foreach($divisions as $division)
                    <div class="checkbox-item">
                        <input type="checkbox" id="division_{{ $division->id }}" name="relevant_divisions[]" 
                               value="{{ $division->id }}" {{ in_array($division->id, old('relevant_divisions', [])) ? 'checked' : '' }}>
                ...
                @foreach($jobPositions as $jobPosition)
                    <div class="checkbox-item" data-division-id="{{ $jobPosition->division_id }}">
                        <input type="checkbox" id="job_{{ $jobPosition->id }}" name="relevant_job_positions[]" 
                               value="{{ $jobPosition->id }}" {{ in_array($jobPosition->id, old('relevant_job_positions', [])) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterJobPositions() {
        ...
        if (selectedDivisionIds.length === 0) {
            // Tampilkan semua jika belum ada divisi dipilih
    ...
    // Jalankan saat awal load jika sudah ada nilai lama
    filterJobPositions();
});
</script>
@endsection
